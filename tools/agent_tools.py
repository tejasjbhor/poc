...
async def classify_requirement(
    ...
    if any(
        ...
        w in combined
        for w in ["maintenance", "repair", "remote handling", "telemanipulator"]
    ):
        domain_tag = "remote_maintenance"
        priority_dbs = ["IAEA SSG-28", "IEC 60780", "MIL-STD-1472"]
    elif any(
        w in combined
        for w in ["safeguard", "accountancy", "material", "nuclear material"]
    ):
    ...
    if domain_tag == "nuclear_criticality_safety":
        ...
        tech_query = (
            f"radiation monitoring system dosimetry technology nuclear facility"
        )
        ...
        tech_query = (
            f"nuclear instrumentation control system DCS remote operations technology"
        )
        ...
        tech_query = (
            f"remote handling telemanipulator nuclear hot cell maintenance technology"
        )
...
async def classify_requirement_json(input_json: str) -> str:
    ...
    if any(
        w in t
        for w in ["criticality", "fissile", "neutron", "subcritical", "criticité"]
    ):
        domain_tag = "nuclear_criticality_safety"
        ...
        domain_tag = "radiation_protection"
        std_q = (
            "radiation protection occupational exposure standard IAEA GSR Part 3 ICRP"
        )
        tech_q = (
            "radiation monitoring dosimetry system personal detector nuclear facility"
        )
        ...
        arxiv_q = (
            "PUREX solvent extraction pulsed column mixer settler nuclear reprocessing"
        )
    elif req_type == "regulatory" or any(
        w in t for w in ["asn", "inb", "legifrance", "regulatory", "arrêté", "décret"]
    ):
        domain_tag = "regulatory_compliance"
...
async def search_standards_web(query: str, max_results: int = 5) -> str:
    ...
    try:
        ...
        log.info(
            "standards_search.done", query=query[:60], results=len(output["results"])
        )
...
async def search_technologies_web(query: str, max_results: int = 5) -> str:
    ...
    try:
        ...
        log.info("tech_search.done", query=query[:60], results=len(output["results"]))
        return json.dumps(output)
    except Exception as exc:
        return json.dumps({"error": str(exc), "results": []})
